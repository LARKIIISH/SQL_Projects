...
(
SELECT 
    production_company, movie_id, avg_rating
FROM
    movie AS m
        INNER JOIN
...
SELECT 
    country, SUM(total_votes) AS votes
FROM
    movie AS m
        INNER JOIN
    ratings AS r ON m.id = r.movie_id
...
ON rm.name_id = n.id
WHERE 
    rm.category = "actor" AND country = "India"
GROUP BY 
    actor_name
HAVING 
    movie_count >= 5;
...
ON rm.name_id = n.id
WHERE 
    rm.category = "actress" AND country = "India" and languages LIKE "%Hindi%"
GROUP BY 
    actress_name
HAVING 
    movie_count >= 3
...
WITH rank_by_genre AS 
(SELECT genre,
		RANK() OVER(ORDER BY count(m.id) DESC) AS rank_g
		FROM 
            genre AS g 
                INNER JOIN 
...
SELECT id,
       ...
       worlwide_gross_income,
		CASE
			WHEN worlwide_gross_income LIKE ("$%") THEN cast(trim(REPLACE(worlwide_gross_income, "$","")) AS UNSIGNED)
			ELSE cast(trim(REPLACE(worlwide_gross_income,"INR","")) AS UNSIGNED)/81.85
...
SELECT rg.genre,
	   ...
	   worlwide_gross_income,
       income_in_usd,       
       DENSE_RANK() OVER(PARTITION BY year ORDER BY income_in_usd DESC) AS movie_rank,
       ROW_NUMBER() OVER(PARTITION BY title ORDER BY genre) AS multiple_genre
from 
    gross_convrtd_dollar AS gcd 
        INNER JOIN 
	genre AS g ON gcd.id =g.movie_id
        INNER JOIN 
    rank_by_genre AS rg ON g.genre= rg.genre
WHERE
    rank_g <=3 
)
 SELECT genre,
 ...
 FROM 
     rank_year_wise
 WHERE 
     movie_rank<=5 AND multiple_genre=1
 ORDER BY
     year, 
     movie_rank;
...
FROM 
	...
	ratings AS r ON m.id = r.movie_id
WHERE 
    languages LIKE "%,%" AND median_rating >= 8 AND production_company IS NOT NULL
GROUP BY 
    production_company
LIMIT 
    2;
...
SELECT 
	...
	RANK() OVER(ORDER BY COUNT(title) DESC,sum(total_votes) DESC) AS actress_rank
FROM 
    names n 
        INNER JOIN 
...
WITH modified_movie AS
(
SELECT
    movie_id,
    name_id, 
    title, 
    date_format(date_published,"%y%m%d") AS old_date,
    duration,
	LEAD(date_published,1) OVER( PARTITION BY dm.name_id ORDER BY date_published ) AS next_movie_date
FROM 
    movie AS m 
...
director_master_data AS
( 
SELECT 
    name AS director_name,
	count(title) AS no_of_movies,
	round(AVG(datediff(date_format(next_movie_date, "%y%m%d"), old_date))) AS avg_inter_movie_days,
...
FROM
    names AS n 
        INNER JOIN modified_movie AS m
ON m.name_id=n.id
	    INNER JOIN ratings AS r
...
SELECT
	...
	total_duration
FROM 
    director_master_data AS dmd 
        INNER JOIN names AS n
ON dmd.director_name=n.name
ORDER BY
    no_of_movies DESC,
    avg_rating*total_votes DESC
LIMIT 9;